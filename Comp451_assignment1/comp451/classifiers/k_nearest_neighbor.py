...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            #(X_train-X_test)^2 = X_train^2 + X_test^2 - 2*X_train*X_test
        dists = np.sqrt(np.sum(X ** 2, axis=1).reshape((X.shape[0], 1)) +
                        np.sum(self.X_train ** 2, axis=1).reshape((1, self.X_train.shape[0])) -
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #np.sum(np.sqrt(np.sum(X ** 2, axis=1).reshape((X.shape[0], 1)) +
        #                np.sum(self.X_train ** 2, axis=1).reshape((1, self.X_train.shape[0])) -
        #                2 * X.dot(self.X_train.T)),axis=1)
        #(np.sqrt(np.sum(self.X_train,axis = 1).reshape((1, self.X_train.shape[0])))
        #- np.sqrt(np.sum(X,axis=1).reshape((X.shape[0],1)))).dot((np.sqrt(np.sum(self.X_train,axis = 1).reshape((1, self.X_train.shape[0])))
        #+ np.sqrt(np.sum(X,axis=1).reshape((X.shape[0],1)))))


        ##ARBITRARY
        X_train_sqrt = np.sqrt(self.X_train)

        X_sqrt = np.sqrt(X)

        dists = (np.sum(self.X_train, axis=1).reshape((1, self.X_train.shape[0]))
         - np.sum(X,axis=1).reshape((X.shape[0],1))) * (np.sum(self.X_train,axis = 1).reshape((1, self.X_train.shape[0]))
         + np.sum(X,axis=1).reshape((X.shape[0],1)))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            for j in range(k):
                closest_y.append(self.y_train[indices[j]])
            ...
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            classVotes = {}
            for label in range(len(closest_y)):
                vote = closest_y[label]
            ...
            tie = False
            maxVote = max(classVotes.keys(), key=(lambda k: classVotes[k]))
            y_pred[i] = maxVote
